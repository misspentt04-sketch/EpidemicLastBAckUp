...
async def cmd_top_rb(message: types.Message, db):
    await show_top_rb(message, db, page=1, per_page=10, is_callback=False)

async def show_top_rb(message: types.Message, db, page: int, per_page: int, is_callback: bool = False):
    ...
    if total_count == 0:
        text = "🔄 <b>ТОП ПО ПЕРЕРОЖДЕНИЯМ</b>\n\n<i>Пока никто не совершил перерождение.</i>"
        if is_callback:
    ...
    query = """
        SELECT Lab.lab_id, lab_name, rebirth_level, last_farm
        FROM Lab
...
async def top_rb_page(callback: types.CallbackQuery, db):
    ...
    await show_top_rb(callback.message, db, page, per_page, is_callback=True)
