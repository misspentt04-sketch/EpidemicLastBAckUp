...
class RequestsRepoBiowar:
    ...
    async def get_my_infected(self, user_id: int):
        ...
        res = await self.select_one(query, user_id)
    ...
    async def get_victims_food(self, user_id: int):
        query = 'SELECT COALESCE(SUM(v.victim_bio_resource_earn), 0) * (1 + COALESCE(l.rebirth_level, 0) * 0.10) FROM Victims v LEFT JOIN Lab l ON l.lab_id = v.victims_owner_id WHERE v.victims_owner_id = %s GROUP BY l.rebirth_level;'
        res = await self.select_one(query, user_id)
        return int(res) if res else 0
    ...
    async def game_mute_add(self, user_id: int, admin_id: int, reason: str, expire_ts: int):
        """Добавляет игровой мут (АС)"""
        query = """
        ...
        """
        await self.execute(query, user_id, admin_id, reason, expire_ts)

    async def game_mute_cancel(self, user_id: int):
        """Снимает игровой мут (АС)"""
        query = "DELETE FROM GameMute WHERE user_id = %s;"
        await self.execute(query, (user_id,))

    async def bio_mute_add(self, user_id: int, admin_id: int, reason: str, expire_ts: int):
